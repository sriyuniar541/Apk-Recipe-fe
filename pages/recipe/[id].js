...
export default function DetailRecipe() {
 ...
 const saveRecipe = (e) => {
  e.preventDefault()  
    ...
    .then(res => {console.log(res, 'saved data success')
 ...
 const likerecipe = (e) => {
    ...
    .then(res => {console.log(res, 'like data success')
    ...
    <div>
      ...
      <div className='container'>
        <><div className='text-center'>
            <h1 className='mb-4' style={{color:'#2E266F'}}>Loream Sandwich {data?data.title:'data not found'}</h1>
            <img src={data?data.photo:'data not found'} alt='' style={{width:'600px',height:'450px'}} className='img-fluid'/>
          </div>
          <div className='d-flex justify-content-center mt-2'>
          <button onClick={saveRecipe} className='btn btn-white border-warning me-2'>
              <img src='/spg.png' alt='' style={{width:'30px',height:'30px'}}/>
          </button>
          <button onClick={likerecipe} className='btn btn-white border-warning'>
              <img src='/like.png' alt='' style={{width:'30px',height:'30px'}}/>
          </button>
          </div>
          <div className='col-11 offset-1 mt-5'>
              ...
              <Link href={`/vidio/${data.id}`}><h4 className='mt-5'>Vidio Step</h4>
                  <img src='/vd.png' alt='' style={{ width: '20%' }} className='mb-3' /><br />
                  <img src='/vd.png' alt='' style={{ width: '20%' }} className='mb-3' /><br />
                  <img src='/vd.png' alt='' style={{ width: '20%' }} className='mb-3' />
              </Link>
              <div className='col-12 text-center'>
                <textarea className="form-control mb-3 border-white" id="exampleFormControlTextarea1" rows="10" placeholder='Comment :' style={{ backgroundColor: '#F6F5F4' }} name='comment' value={comment} onChange={((e)=>{ setComment(e.target.value)})}/>
                {/* <input name='comment' value={comment} onChange={((e)=>{ setComment(e.target.value)})}/>  */}
                <button className='btn btn-warning text-white col-3' onClick={postComment}>Post</button>
              </div>
            </div></>
              ...
              <div style={{ marginBottom: '20%', marginTop: '5%' }}>
                  ...
                  {getcomment.map((p,i)=> (
                    <><div className='d-flex mb-5' style={{lineHeight:'20%'}}>
                        <img src={p.user_recipe_photo} alt=''className='mb-3' style={{width:'50px', borderRadius:'50%'}} />
